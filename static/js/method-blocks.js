...
const PY_STR_METHOD_DEFS = [
  ...
  { type: "py_str_isupper", label: "в верхнем регистре?", py: "isupper()", bool: true },
  {
    ...
    label: "разделить",
    py: "split",
    args: [{ name: "SEP", check: "String", label: "по" }],
    optionalArg: "SEP",
  },
  {
    type: "py_str_find",
  ...
  },
  {
    ...
    args: [
      ...
      { name: "NEW", check: "String", label: "на" },
    ],
  },
  {
    type: "py_str_join",
...
const PY_LIST_STMT_DEFS = [
  ...
  },
  {
    ...
    py: "insert",
    args: [
      { name: "INDEX", check: "Number", label: "на место" },
      { name: "VALUE", label: "" },
    ],
  },
  {
...
const PY_LIST_EXPR_DEFS = [
  {
    ...
    label: "извлечь",
    py: "pop",
    args: [{ name: "INDEX", check: "Number", label: "с позиции" }],
    optionalArg: "INDEX",
  },
  {
    type: "py_list_count",
...
function buildStrMethodJson(def) {
  if (def.join) {
    return {
      type: def.type,
      message0: "разделитель %1 → соединить %2",
      args0: [
...
function buildListStmtJson(def) {
  ...
  return {
    ...
    nextStatement: null,
    style: "list_blocks",
    tooltip: "Метод списка: " + def.label,
  };
}
